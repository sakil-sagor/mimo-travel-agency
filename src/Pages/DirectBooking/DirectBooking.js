...
const DirectBooking = (props) => {
    ...
    const handelRegister = e => {
        ...
        const newServices = { name, email, phone, packName, address, fromDate, toDate, status }
        ...
        fetch(' https://infinite-island-58921.herokuapp.com/orders', {
            ...
            .then(data => {
                if (data.insertedId) {
                    ...
                    getPhone.current.value = getFromDate.current.value = getToDate.current.value = getAddress.current.value = '';
                }
            })
        e.preventDefault();
                        ...
                        <div className='col-span-12 md:col-span-8 bg-pink-200 rounded-xl shadow-2xl'>
                            <div className=" p-8 m-auto">
                                <form onSubmit={handelRegister}>
                                    ...
                                    <div className="mb-4">
                                        <label htmlFor="country" className="text-gray-500 "> Select Your Package</label>
                                        <br />
                                        <input type="text" id="country" value={`${name},  ${country} ` || ''} ref={getPackName} required className="bg-white py-2 px-2 w-full rounded-2xl mt-2" />
                                    </div>
                                    <div className="mb-4">
                                        <label htmlFor="address" className="text-gray-500 "> Your Address</label>
                                        <br />
                                        <input type="text" id="address" ref={getAddress} required className="bg-white py-2 px-2 w-full rounded-2xl mt-2" />
                                    </div>
